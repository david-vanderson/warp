...
(define (nebula-region r)
  ...
  (define sep 500.0)
  (define sep/2 (/ sep 2))
  (for*/list ((x (in-range (+ rx sep/2) (- (+ rx rw 1.0) sep/2) sep))
              (y (in-range (+ ry sep/2) (- (+ ry rh 1.0) sep/2) sep))
...
(define (mine-region r f)
  ...
  (for*/list ((x (in-range (+ rx sep/2) (- (+ rx rw 1.0) sep/2) sep))
              (y (in-range (+ ry sep/2) (- (+ ry rh 1.0) sep/2) sep))
              #:when (send r in-region? x y))
    (define xd (random-between (* sep -0.2) (* sep 0.2)))
    (define yd (random-between (* sep -0.2) (* sep 0.2)))
    (f (+ x xd) (+ y yd))))
