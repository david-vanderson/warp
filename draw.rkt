           ...
           ((ship? o)
            ...
            (when (warp-charging? space o)
              ...
              (define a (* fowa (+ 0.3 (* 0.7 (/ (warp-energy w)
                                                 (warp-threshold w))))))
              (prepend! spr (obj-sprite o csd center scale LAYER_MAP
                                        sym size a r
            ...
            (cond
              ((warping? o)
               (define age (obj-age space o))
               (define anim? ((remainder age 1000) . < . 500))
               (define frame (quotient age 100))
               (define sym (if anim?
                               (anim-frame-sym "warping-static" 0)
                               (anim-frame-sym "warping" frame)))
               (define size (* 2.0 (/ (exact->inexact (ship-sprite-size o))
                                      (sprite-size csd sym))))
               (prepend! spr (obj-sprite o csd center scale layer-effects
                                         sym size fowa (obj-r o)
                                         (make-color 0 0 0 1.0))))
              (eng
